...
A streaming A2A chat agent that looks up library documentation from live package registries.
It runs a short tool loop against an OpenAI-compatible model and must back claims with tool
output rather than model memory. Answers are expected to include the exact version reported by
the registry.

Example requests:

```text
Look up the latest version of the serde crate
Find documentation for the express package on npm
What does FastAPI depend on?
Show usage examples from tokio-rs/tokio
Compare serde and bincode for binary encoding in Rust
```

The agent looks up crates, npm packages, PyPI projects, GitHub READMEs, and web results. It
does not write, host, or edit your project's documentation.

## Tools

The model can call these tools, with at most four tool-call rounds per request:

- `crates_io` searches crates.io (downloads by default) and returns name, version, description,
  and docs links.
- `crate_info` returns version history, features, dependencies, and a docs.rs link for one crate.
- `npm_search` searches npm for JavaScript and TypeScript packages.
- `npm_package` returns versions, dependencies, repository, and a README excerpt for one package.
- `pypi_package` returns version, description, dependencies, and project links from PyPI.
- `github_readme` fetches the README of a public `owner/repo` repository.
- `web_search` is a fallback for comparisons, tutorials, or questions the registries do not
  cover. Registry data is preferred.

GitHub and registry requests use the public APIs without a token. Rate limits are tighter, and
...
└── src/
    ...
    ├── tools.rs            # crates.io, npm, PyPI, GitHub, and web-search tools
...
- `OTEL_SERVICE_NAME` defaults to `nasiko-docs-agent`.
...
Remove a vault secret with `nasiko secrets rm <KEY>`. Add `--agent docs` when removing an
...
nasiko ps --json
nasiko logs docs
nasiko logs docs -n 200
nasiko logs docs -n 200 -f
nasiko stop docs
nasiko start docs
nasiko restart docs
nasiko scale docs 2
...
```sh
nasiko chat docs "Find documentation for the express package on npm"
```
...
```sh
nasiko sessions
nasiko chat --agent docs --resume <session-id>
nasiko chat --agent docs \
  --session-id <session-id> \
  "Now look up FastAPI on PyPI."
```
...
```sh
nasiko rollback --name docs
nasiko rollback --name docs --version 1.0.0
```
...
### Registry or GitHub lookups fail or return rate-limit errors

The tools call public APIs without a token. Wait and retry, or ask about a public package.
Private GitHub repositories are not accessible.

### The answer invents an API signature

The system prompt requires a tool call before answering. If a lookup failed, retry with the
exact crate or package name rather than a vague description.
