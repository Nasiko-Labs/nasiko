...
// ─── HTTP plumbing (shared by every tool) ────────────────────────────────────
...
// ─── GitHub fetch + render (one concern per function) ─────────────────────────
...
// ─── Per-repo bodies (one repo each; the tools fan these out) ─────────────────
...
// ─── Public tools (the agent's callable surface) ──────────────────────────────
...
pub async fn get_commit(
    ...
    get_commit_body(owner, name, commit_hash.as_deref())
        .await
...
pub async fn read_file(
    ...
    let (owner, name) = split_repo(&repo).map_err(GitHubToolError)?;
    read_file_body(owner, name, &path, &git_ref)
        .await
        .map_err(GitHubToolError)
}
...
pub async fn find_references(
    ...
    let (owner, name) = split_repo(&repo).map_err(GitHubToolError)?;
    find_references_body(owner, name, &symbol)
        .await
        .map_err(GitHubToolError)
}
