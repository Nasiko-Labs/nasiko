...
styles.replaceSync(`@scope (your-agents-page) {
  :scope { display: block; }
  .env-row {
    display: grid;
  ...
  .env-row input:focus { outline: none; border-color: var(--color-primary); }
  .env-remove {
    background: none;
    border: none;
    cursor: pointer;
  ...
  .env-remove:hover { background: var(--color-bg-elevated); }
  .modal-section { margin-bottom: var(--space-md); }
  .modal-section h3 {
    font-size: var(--font-size-sm);
    font-weight: 600;
    color: var(--color-text-main);
  ...
  .add-env-btn:hover, .import-btn:hover {
    border-color: var(--color-primary);
    color: var(--color-primary);
  }
  .import-btn { margin-left: var(--space-sm); }
  .secret-chips {
    display: flex;
    flex-wrap: wrap;
    gap: var(--space-xs);
  ...
  .btn-deploy:hover:not(:disabled) { background: var(--color-primary-hover); }
