...
static CATEGORY_PATTERNS: LazyLock<Vec<(RequestType, Vec<Regex>)>> = LazyLock::new(|| {
...
// --------------------------------------------------------------------------
// 2. Feedback signal — port of classifier/signals.rs
// --------------------------------------------------------------------------

static NEGATIVE_SIGNALS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    [
        r"(?i)\bthat'?s wrong\b",
...
static POSITIVE_SIGNALS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    [
        r"(?i)\bthanks?( you)?\b",
        r"(?i)\bperfect\b",
        r"(?i)\bexactly\b",
