...
mod tests {
    ...
    fn http_status_mapping() {
        ...
        assert_eq!(
            McpError::ToolApprovalRequired("x".into()).http_status(),
            428
        );
    ...
    fn json_rpc_code_mapping() {
        assert_eq!(
            McpError::BadRequest("x".into()).json_rpc_code(),
            codes::INVALID_PARAMS
        );
        assert_eq!(
            McpError::NotFound("x".into()).json_rpc_code(),
            codes::METHOD_NOT_FOUND
        );
        assert_eq!(
            McpError::ToolBlocked("x".into()).json_rpc_code(),
            codes::TOOL_BLOCKED
        );
        assert_eq!(
            McpError::ToolApprovalRequired("x".into()).json_rpc_code(),
            codes::TOOL_ASK
        );
        assert_eq!(
            McpError::Forbidden("x".into()).json_rpc_code(),
            codes::TOOL_BLOCKED
        );
        assert_eq!(
            McpError::Conflict("x".into()).json_rpc_code(),
            codes::INVALID_PARAMS
        );
        assert_eq!(
            McpError::Internal("x".into()).json_rpc_code(),
    ...
    fn internal_causes_are_redacted_client_facing() {
        ...
        let msg = McpError::Internal("secret db dsn".into())
            .to_json_rpc()
            .message;
        assert_eq!(msg, "internal error");
        // But client-safe errors keep their message.
        assert_eq!(
            McpError::ToolBlocked("blocked!".into())
                .to_json_rpc()
                .message,
            "blocked!"
        );
    ...
    fn sensitive_variants_are_redacted_on_agent_path() {
        ...
        assert_eq!(
            McpError::Crypto("decrypt failed for key X".into())
                .to_json_rpc()
                .message,
            "internal error"
        );
        assert_eq!(
            McpError::Crypto("decrypt failed for key X".into()).client_message(),
            "internal error"
        );
        // Backend/Composio/Oauth detail is redacted on the agent JSON-RPC path.
        assert_eq!(
            McpError::Backend("upstream at 10.0.1.5 refused".into())
                .to_json_rpc()
                .message,
            "backend request failed"
        );
        assert_eq!(
            McpError::Composio("composio 500".into())
                .to_json_rpc()
                .message,
            "backend request failed"
        );
        assert_eq!(
            McpError::Oauth("bad token endpoint".into())
                .to_json_rpc()
                .message,
            "authorization error"
        );
        ...
        assert_eq!(
            McpError::Oauth("token exchange failed (HTTP 500): <body>".into()).client_message(),
            "authorization error"
        );
        // …but Backend detail is preserved for management responses (safe diagnostics).
        assert_eq!(
            McpError::Backend("could not reach MCP server".into()).client_message(),
            "backend error: could not reach MCP server"
        );
    }
    ...
    fn client_message_redaction_table_for_every_variant() {
        ...
        assert_eq!(
            McpError::NotFound("conn x".into()).client_message(),
            "not found: conn x"
        );
        assert_eq!(
            McpError::BadRequest("bad url".into()).client_message(),
            "bad request: bad url"
        );
        assert_eq!(
            McpError::Unauthorized("no token".into()).client_message(),
            "unauthorized: no token"
        );
        assert_eq!(
            McpError::Forbidden("no access".into()).client_message(),
            "forbidden: no access"
        );
        assert_eq!(
            McpError::Conflict("dup".into()).client_message(),
            "conflict: dup"
        );
        assert_eq!(
            McpError::ToolBlocked("blocked!".into()).client_message(),
            "blocked!"
        );
        assert_eq!(
            McpError::ToolApprovalRequired("ask!".into()).client_message(),
            "ask!"
        );
        assert_eq!(
            McpError::NotConfigured("no key".into()).client_message(),
        ...
        assert_eq!(
            McpError::Composio("composio 500".into()).client_message(),
            "composio error: composio 500"
        );
        ...
        assert_eq!(
            McpError::Oauth("token exchange failed (HTTP 502): <internal-host-detail>".into())
                .client_message(),
            "authorization error"
        );
        ...
        assert_eq!(
            McpError::Database(sqlx::Error::RowNotFound).client_message(),
            "internal error"
        );
        let io_err = std::io::Error::other("boom");
        let redis_err: redis::RedisError = io_err.into();
        assert_eq!(
            McpError::Redis(redis_err).client_message(),
            "internal error"
        );
        let serde_err = serde_json::from_str::<serde_json::Value>("not json").unwrap_err();
        assert_eq!(
            McpError::Serde(serde_err).client_message(),
            "internal error"
        );
        assert_eq!(
            McpError::Crypto("decrypt failed for key X".into()).client_message(),
        ...
        assert_eq!(
            McpError::Internal("secret db dsn".into()).client_message(),
            "internal error"
        );
    }
    ...
    async fn http_variant_is_redacted_on_both_surfaces() {
        let err1 = reqwest::Client::new()
            .get("http://127.0.0.1:1/")
            .send()
            .await
            .unwrap_err();
        assert_eq!(
            McpError::Http(err1).to_json_rpc().message,
            "backend request failed"
        );
        // reqwest::Error isn't Clone — fetch a fresh error for the second check.
        let err2 = reqwest::Client::new()
            .get("http://127.0.0.1:1/")
            .send()
            .await
            .unwrap_err();
        assert_eq!(
            McpError::Http(err2).client_message(),
            "backend request failed"
        );
    }
}
