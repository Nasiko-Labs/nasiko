...
pub struct McpConfig {
    ...
    /// Base URL for the OAuth 2.1 browser redirect — distinct from
    /// `gateway_public_url` on purpose, see `Config::mcp_oauth_redirect_base_url`'s
    /// doc comment for why. Falls back to `gateway_public_url` when unset.
...
mod tests {
    ...
    fn oauth_redirect_uri_falls_back_to_gateway_public_url_when_unset() {
        let cfg = McpConfig { gateway_public_url: Some("http://host.docker.internal:8080/api/mcp".into()), ..base_config() };
        assert_eq!(
    ...
    fn oauth_redirect_uri_trims_trailing_slash_on_either_source() {
        let cfg = McpConfig { oauth_redirect_base_url: Some("http://localhost:8080/".into()), ..base_config() };
        assert_eq!(cfg.oauth_redirect_uri().as_deref(), Some("http://localhost:8080/oauth/callback"));
    }
}
