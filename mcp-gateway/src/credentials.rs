...
async fn build_server_config(
    ...
    match auth_type {
        ...
        "bearer" | "basic" => match conn.and_then(|c| c.encrypted_credential.as_deref()) {
            Some(enc) => {
                let Some(value) = decrypt_or_skip(crypto, enc, connector) else { return Ok(None) };
        ...
        "url_param" => {
            ...
            let Some(enc) = conn.and_then(|c| c.encrypted_credential.as_deref()) else { return Ok(None) };
            let Some(value) = decrypt_or_skip(crypto, enc, connector) else { return Ok(None) };
...
pub async fn verify_connector_live(state: &McpState, user_id: Uuid, connector: &McpConnector) -> VerifyOutcome {
    let crypto = SecretsCrypto::for_user(user_id);
    let conn = match repo::get_user_connection(&state.db, user_id, connector.id).await {
        Ok(c) => c,
        Err(e) => return VerifyOutcome { verified: false, error: Some(format!("failed to load credential: {e}")) },
    };
    ...
    let cfg = match build_server_config(state, &crypto, user_id, connector, conn.as_ref()).await {
        ...
        Ok(None) => {
            return VerifyOutcome {
                verified: false,
                error: Some("no usable credential is configured for this connector yet".to_string()),
            };
        }
        Err(e) => return VerifyOutcome { verified: false, error: Some(format!("failed to prepare request: {e}")) },
    };

    let provider = GenericMcpProvider::new(state.guarded_http_client.clone(), state.http_client.clone());
    match provider.list_tools(&cfg, LIST_TIMEOUT, None).await {
        Ok(_) => VerifyOutcome { verified: true, error: None },
        Err(e) => VerifyOutcome { verified: false, error: Some(e.to_string()) },
    }
}
...
pub async fn register_credential(
    ...
    let (status, error) = if outcome.verified { ("active", None) } else { ("failed", outcome.error.as_deref()) };
