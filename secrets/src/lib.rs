...
impl SecretsCrypto {
    ...
    fn derive(info: &[u8]) -> Self {
        ...
        hk.expand(info, &mut derived).expect("HKDF expand failed — info too long");
    ...
    fn load_master_key() -> Vec<u8> {
        ...
        let bytes = BASE64.decode(&b64).expect("SECRETS_ENCRYPTION_KEY: invalid base64");
        assert_eq!(bytes.len(), 32, "SECRETS_ENCRYPTION_KEY must decode to exactly 32 bytes");
        bytes
    }
}
