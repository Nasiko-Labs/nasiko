...
impl SecretsCrypto {
    ...
    /// Derive a tenant-scoped cipher from the master key (HKDF-SHA256, info =
    /// tenant UUID). Used by `ee/multi-tenant` to encrypt a tenant cluster's
    /// FinOps-poller service-account credential at rest.
