-- =============================================================================
-- MCP Gateway v2 — unified connectors + owner-controlled sharing
-- =============================================================================
--
-- Full replacement of the v1 MCP schema (018_mcp_schema.sql). v1 split Composio
-- toolkits and custom MCP servers into two tables scoped by is_platform/user_id
-- and keyed everything by server_name (string). v2 unifies both into a single
-- `mcp_connectors` registry (discriminated by provider_type), keys everything by
-- connector id (UUID), and adds owner-controlled sharing (`mcp_connector_grants`,
-- mirroring the platform's existing `agent_grants` shape).
--
-- This is a destructive, forward-only replacement — there is no data migration
-- from v1 (pre-GA; any live v1 connections must reconnect). Enum-like columns use
-- TEXT + CHECK (matches the platform's build_jobs.status / agent-grants pattern)
-- so adding a provider or auth type later is a CHECK change, not an ALTER TYPE.
--
-- Audited fixes baked in (see MCP_GATEWAY_TECHNICAL_DESIGN.md §18):
--   #1 tool-routing prefix derives from id, never name (enforced in crate code).
--   #2 revoking a grant deletes the grantee's connection row (enforced in crate).
...
-- derived from `id` in crate code — audited fix #1).
CREATE TABLE mcp_connectors (
    ...
    -- the wrong provider's (audited fix #3) — an empty, unusable connector cannot
    -- be inserted.
...
-- exactly why the tool-routing prefix comes from id, not name (fix #1).
...
-- =============================================================================
-- Mirrors agent_grants (001_schema.sql): grant_type 'user' targets a specific
...
-- code (audited fix #2), not by the schema.
...
-- =============================================================================
-- One row per (user, connector), covering every auth shape uniformly. Replaces
-- v1's mcp_connections + mcp_oauth_tokens + mcp_user_credentials. Never shared,
-- even when the connector itself is. No credential_type column — the format is
-- read by joining mcp_connectors.auth_type (audited fix #4). All *encrypted_*
-- columns are stored ENCRYPTED (SecretsCrypto::for_user) and are write-only —
