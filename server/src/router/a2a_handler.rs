...
pub async fn a2a_handler(
    ...
    if use_router {
        ...
        match state.routing_engine.route(route_req, &state.db).await {
            ...
            Err(RouterError::NoAgentsAvailable) => Err(A2aHandlerError::NoAgents),
            Err(e) => Err(A2aHandlerError::Internal(e.to_string())),
        }
...
pub async fn a2a_upload_handler(
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|e| A2aHandlerError::InvalidRequest(format!("multipart error: {e}")))?
    {
        ...
        let bytes = field
            .bytes()
            .await
            .map_err(|e| A2aHandlerError::InvalidRequest(format!("field read error: {e}")))?;

        if field_name == "query" {
            query = String::from_utf8(bytes.to_vec())
                .map_err(|_| A2aHandlerError::InvalidRequest("query must be valid UTF-8".into()))?;
        } else {
    ...
    let user_id: uuid::Uuid = claims.sub.parse().unwrap_or(uuid::Uuid::nil());
