...
pub fn router_routes(a2a_limiter: crate::rate_limit::RateLimiter) -> axum::Router<crate::state::AppState> {
    let dispatch_routes = axum::Router::new()
        .route("/orchestrator/a2a", axum::routing::post(a2a_dispatch_handler))
        .route("/orchestrator/a2a/upload", axum::routing::post(a2a_upload_handler))
        .layer(axum::middleware::from_fn_with_state(a2a_limiter, crate::rate_limit::limit_by_user));

    axum::Router::new()
        .merge(dispatch_routes)
        .route("/orchestrator/stats", axum::routing::get(router_stats_handler))
}