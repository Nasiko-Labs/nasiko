...
pub async fn require_delegation(mut req: Request, next: Next) -> Response {
    ...
    let Some(token) =
        req.headers().get(HEADER_AGENT_TOKEN).and_then(|v| v.to_str().ok()).map(str::to_owned)
    else {
    ...
    req.extensions_mut().insert(Claims { sub: user_id, username: String::new(), is_superuser: false });
    next.run(req).await
}
...
pub async fn mcp_gateway(
    ...
    let traceparent = headers.get(nasiko_flow::TRACEPARENT_HEADER).and_then(|v| v.to_str().ok());
    let method = body.get("method").and_then(|m| m.as_str()).unwrap_or("").to_string();
    let tool_name =
        body.get("params").and_then(|p| p.get("name")).and_then(|v| v.as_str()).unwrap_or("").to_string();

    let started = std::time::Instant::now();
    let Some(result) = protocol::handle_request(&state.mcp, user_id, agent_id, &body, traceparent).await else {
    ...
    if method == "tools/call" {
        ...
        record_tool_usage(&state, user_id, agent_id, &tool_name, latency_ms, success, None);
            ...
            && let Some(flow_ctx) = traceparent.and_then(nasiko_flow::FlowContext::from_traceparent)
        {
            ...
            let server = result
                ...
                .map(str::to_string)
                .unwrap_or_else(|| {
                    tool_name.split_once("__").map(|(s, _)| s.to_string()).unwrap_or_else(|| "composio".to_string())
                });
            state
                .flow_events
                .publish(
...
fn record_tool_usage(
    ...
    state.genai_metrics.record_tool_call(latency_ms as f64 / 1000.0, tool_name, &agent_id.to_string());
...
fn acting_agent_id(headers: &HeaderMap, user_id: &str) -> Option<Uuid> {
    ...
    let (delegated_user_id, agent_id) = nasiko_auth::jwt::validate_delegation_token(&jwt_secret, token).ok()?;
