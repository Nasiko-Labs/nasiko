...
fn toolkit_logo_url(toolkit: &str) -> Option<&'static str> {
    ...
    Some(match toolkit {
        ...
        "googlecalendar" => "https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/googlecalendar.svg",
...
pub async fn seed_toolkits_if_configured(state: &AppState) {
    ...
    let Some(provider) = &state.mcp.providers.composio else { return };
    ...
    info!(count = toolkit_names.len(), "bulk-syncing tools for newly seeded toolkits");
    let tools_by_toolkit = composio.list_tools_for_toolkits(&toolkit_names).await;
    for (toolkit, tools) in &tools_by_toolkit {
        let Some(cid) = newly_seeded.get(toolkit) else { continue };
        if tools.is_empty() {
            continue;
        }
