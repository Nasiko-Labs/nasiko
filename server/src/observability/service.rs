...
/// Normalize a Tempo trace ID to exactly 32 lowercase hex chars.
/// Tempo search sometimes strips leading zeros (e.g. "04dfe7c..." → "4dfe7c..."),
/// while the W3C traceparent stored in Redis always has the full 32 chars.
/// Padding ensures both forms compare equal during deduplication.
fn normalize_trace_id(id: &str) -> String {
    format!("{:0>32}", id.to_lowercase())
}
...
///
/// Handles two formats emitted by different OpenTelemetry instrumentation versions:
///
/// **v1 format** (`opentelemetry-instrumentation-openai` ≤ 1.x):
/// Two separate log records per LLM call — one for prompt, one for completion.
/// `attributes` is an OTLP protobuf JSON array:
/// ```json
/// { "spanId": "abc", "attributes": [{"key": "event.name", "value": {"stringValue": "gen_ai.content.prompt"}}, ...] }
/// ```
///
/// **v2 format** (`opentelemetry-instrumentation-openai-v2` / `opentelemetry-util-genai`):
/// A single EventLogger log record per LLM call containing both input and output.
/// `attributes` is a flat JSON object:
/// ```json
/// { "spanid": "abc", "attributes": { "gen_ai.input.messages": [...], "gen_ai.output.messages": [...] } }
/// ```
fn parse_loki_logs(lines: Vec<String>) -> HashMap<String, SpanContent> {
    ...
    for line in lines {
        ...
        match &entry["attributes"] {
            // ── v2: attributes is a flat JSON object ──────────────────────────
            Value::Object(attrs) => {
                // gen_ai.input.messages — array of {role, parts:[{content, type}]}
                if let Some(msgs) = attrs.get("gen_ai.input.messages") {
                    slot.input = serde_json::to_string(msgs).ok();
                }
                // gen_ai.output.messages — same shape
                if let Some(msgs) = attrs.get("gen_ai.output.messages") {
                    slot.output = serde_json::to_string(msgs).ok();
                }
                // Fallback: older string-valued prompt/completion attributes
                if slot.input.is_none() {
                    if let Some(v) = attrs.get("gen_ai.prompt").or_else(|| attrs.get("gen_ai.content.prompt")) {
                        slot.input = v.as_str().map(String::from);
                    }
                }
                if slot.output.is_none() {
                    if let Some(v) = attrs.get("gen_ai.completion").or_else(|| attrs.get("gen_ai.content.completion")) {
                        slot.output = v.as_str().map(String::from);
                    }
                }
            }
            // ── v1: attributes is an OTLP protobuf JSON array ─────────────────
            Value::Array(attrs) => {
                ...
                for attr in attrs {
                    ...
                    match key {
                        "event.name" => event_name = attr["value"]["stringValue"].as_str().map(String::from),
...
impl ObservabilityService {
    ...
    /// Retrieve all trace_ids recorded for a session via the reverse index written by agent_proxy.
    /// Covers pre-built agents whose spans don't carry the session.id attribute.
    async fn redis_traces_for_session(&self, session_id: &str) -> Vec<String> {
        let Ok(mut conn) = self.redis.get_multiplexed_async_connection().await else {
            return vec![];
        };
        let key = format!("nasiko:session:{session_id}:traces");
        redis::cmd("SMEMBERS")
            .arg(&key)
    ...
    pub async fn get_session_details(
        ...
        // Strategy 1: Tempo span attribute set by sitecustomize.py-patched agents.
        let query = format!(r#"{{span.session.id="{session_id}"}}"#);
        let end = Utc::now();
        let start = clamp_tempo_range(end - Duration::days(7), end);
        let mut trace_results = self.tempo.search(&query, Some(start), Some(end), 100).await?;

        // Strategy 2: Redis reverse index written by agent_proxy for pre-built agents
        // (those without sitecustomize.py that never set session.id on spans).
        let redis_trace_ids = self.redis_traces_for_session(session_id).await;
        let existing_ids: std::collections::HashSet<String> =
            trace_results.iter().map(|(id, _, _)| id.clone()).collect();
        for tid in redis_trace_ids {
            if !existing_ids.contains(&tid) {
                trace_results.push((tid, None, None));
            }
        }
        ...
        // Deduplicate trace_ids: Tempo search and Redis may return the same trace with
        // different zero-padding (e.g. Tempo strips leading zeros: "04dfe7c..." → "4dfe7c...").
        // Normalize to 32 hex chars before comparing so both forms are treated as equal.
        ...
        for (trace_id, _, _) in &trace_results {
            ...
            if !seen_trace_ids.insert(normalized) {
                continue;
            }
            let Ok(trace) = self.tempo.get_trace(trace_id).await else { continue };

            // Identify the best root span: one whose parent is not present in this trace.
            // When multiple candidates exist (e.g. a trace shared across several HTTP
            // requests due to OTel context leakage), prefer the first span that has LLM
            // activity (gen_ai.* attributes) or is not a GET request, so that the A2A
            // handler span wins over a GET /.well-known/agent-card.json span.
            let span_ids_set: std::collections::HashSet<&str> =
                trace.spans.iter().map(|s| s.span_id.as_str()).collect();
            let is_candidate = |s: &&nasiko_observability::types::Span| {
                s.parent_span_id
                    .as_ref()
                    .map(|p| !span_ids_set.contains(p.as_str()))
                    .unwrap_or(true)
            };
            let is_get = |s: &&nasiko_observability::types::Span| {
                s.attributes
                    .get("http.method")
                    .and_then(|v| v.as_str())
            ...
            // Prefer non-GET root candidates; fall back to any candidate.
            let root_span = trace.spans.iter().find(|s| is_candidate(s) && !is_get(s))
                .or_else(|| trace.spans.iter().find(is_candidate));
            let Some(root_span) = root_span else { continue };

            // Skip traces with no LLM activity — these are infrastructure traces
            // (agent card GETs, sdk dispatch loops, etc.) with zero token counts.
            let has_llm = trace.spans.iter().any(|s| {
            ...
            let cursor = base64::Engine::encode(
                &base64::engine::general_purpose::STANDARD,
                format!("connection:{}", trace_entries.len()),
            );
    ...
    pub async fn get_span_details(
        ...
        // Extract prompt/completion from span events (set when
        // OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=true).
        // These are more reliable than Loki logs and carry the full message array
        // including tool call arguments and results.
        let event_prompt = span.events.iter()
            .find(|e| e.name == "gen_ai.content.prompt")
            .and_then(|e| {
                e.attributes.get("gen_ai.prompt")
                    .or_else(|| e.attributes.get("gen_ai.content.prompt"))
                    .and_then(|v| v.as_str())
                    .map(String::from)
            });
        let event_completion = span.events.iter()
            .find(|e| e.name == "gen_ai.content.completion")
            .and_then(|e| {
                e.attributes.get("gen_ai.completion")
                    .or_else(|| e.attributes.get("gen_ai.content.completion"))
                    .and_then(|v| v.as_str())
                    .map(String::from)
            });

        // Input priority: span attribute → span event → Loki log
        ...
        // Output priority: span attribute → span event → Loki log
        ...
        // Serialize span events as JSON values for the response.
        // Includes prompt/completion events, tool call records, and any other
        // structured events emitted by the OTel instrumentation.
        let events_json: Vec<Value> = span.events.iter()
            .map(|e| {
                serde_json::json!({
                    "name": e.name,
