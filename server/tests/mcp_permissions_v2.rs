...
async fn default_allow_lists_connector_enabled() {
    ...
    assert_eq!(
        entry["enabled"], false,
        "no row → disabled by default (default-deny)"
    );
...
#[tokio::test]
#[serial]
async fn matrix_absent_access_row_is_default_deny() {
    let server = common::TestServer::start().await;
    let fx = setup_fixture(&server).await;

    // Default-deny: no access row means the connector is disabled,
    // so tools/list returns nothing and tools/call is denied.
    let names = fx.tools_list_names(&server).await;
    assert!(
        names.is_empty(),
        "no access row → connector disabled, no tools visible: {names:?}"
    );

    let res = fx.call(&server, TOOL_OTHER).await;
    assert!(
        res.get("error").is_some(),
        "call must be denied when connector is disabled: {res:?}"
    );
...
async fn matrix_rule_scoped_to_different_connector_does_not_leak() {
    ...
    // Enable the fixture connector so its tools are visible.
    fx.set_connector_enabled(&server, true).await;
