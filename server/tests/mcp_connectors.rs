...
async fn catalog_merges_composio_and_owned_connectors() {
    ...
    // A custom mcp_server returns 0 before any tools have been synced.
    assert_eq!(
        serpapi["tool_count"], 0,
        "mcp_server tool_count must be 0 pre-connect: {}",
        serpapi["tool_count"]
    );
...
async fn probe_detects_auth_types() {
    ...
    .unwrap();
    assert_eq!(
        res.json::<Value>().await.unwrap()["data"]["auth_type"],
        "none"
    );
